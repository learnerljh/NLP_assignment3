...
def customized_greedy_decoding(batch):
    ...
    for timestep in range(MAX_NEW_LENGTH):
        outputs = custom_model(**tokenized_batch, past_key_values=past_key_values)
...
def prefix_caching_decoding(batch):
    prefix_dict = dict()
    if batch[0] in prefix_dict:
        prefix_key_values = prefix_dict[batch[0]]
    else:
        tokenized_prefix = tokenizer(batch[0], return_tensors="pt", padding=True, truncation=True, max_length=128).to('cuda')
        outputs = custom_model(**tokenized_prefix, past_key_values=None)
        prefix_key_values = outputs.past_key_values
    tokenized_batch = tokenizer(batch, return_tensors="pt", padding=True, truncation=True, max_length=128).to('cuda')
    res = tokenized_batch['input_ids']
    start_time = time.time()
    past_key_values = prefix_key_values
    for timestep in range(MAX_NEW_LENGTH):
        outputs = custom_model(**tokenized_batch, past_key_values=None)
        output_tokens = torch.argmax(outputs['logits'][:,-1], dim=-1, keepdim=True)
        tokenized_batch['input_ids'] = torch.cat([tokenized_batch['input_ids'], output_tokens], dim=-1)
        tokenized_batch['attention_mask'] = torch.cat([tokenized_batch['attention_mask'], torch.ones_like(output_tokens)], dim=-1)
        past_key_values = outputs.past_key_values
...
if __name__ == "__main__":
    ...
    for i in range(0, (len(prompt_dataset) + bsz - 1) // bsz):
        ...
        prefix_caching_res, prefix_caching_time = prefix_caching_decoding(batch)
        times[0] += golden_wo_cache_time
        times[1] += custom_time
        times[2] += prefix_caching_time

        assert torch.equal(golden_wo_cache_res, custom_res), "Decoding results are not equal"
        assert torch.equal(golden_wo_cache_res, prefix_caching_res), "Decoding results are not equal"
    ...
    print("Time taken for prefix caching decoding: ", times[2])
