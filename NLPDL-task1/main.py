...
def customized_greedy_decoding(batch):
    ...
    for timestep in range(MAX_NEW_LENGTH):
        tokenized_batch = custom_model.prepare_inputs_for_generation(**tokenized_batch, past_key_values=past_key_values)
...
if __name__ == "__main__":
    ...
    prompt_dataset = []
    with open("NLP_assignment3/NLPDL-task1/data.txt") as f:
        for i in f.readlines():
            i = i.strip()
            prompt_dataset.append(i[:6])
            prompt_dataset.append(i[6:])
